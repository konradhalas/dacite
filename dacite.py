...
class Config:
    ...
    type_transform: Dict[Tuple[type, type], Callable[[Any], Any]] = dc_field(default_factory=dict)
    ...
    for field in fields(data_class):
        ...
        if not is_default:
            if value is not None:
                ...
                if (field.type, type(value)) in config.type_transform:
                    value = config.type_transform[(field.type, type(value))](value)
